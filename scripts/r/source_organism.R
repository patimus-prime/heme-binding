# for my next trick, we'll be grabbing the section of data in the fuckin pdbs
# for the organism the protein is from. we will not be adding the header,
# it gives poor information.
# in each pdb, we need to grab what is after: SOURCE   2 ORGANISM_SCIENTIFIC: HOMO SAPIENS;
# should only be necessary 'ORGANISM SCIENTIFIC: ' bing bang boom
# in each we can also remove the ;
# it may be possible to use substr and begin at ORG and end with ;
# https://stackoverflow.com/questions/39086400/extracting-a-string-between-other-two-strings-in-r
# use that. here we go! we can just grab EVERYTHING from every PDB in the file and use that

#yeah its a lot, so here's my take on it:
# combined results df: first, filter down for the pdb code using the above.
# or maybe use the method we used in pdb-titles_codes

#then filter it down again for the host organism

#combine into one dataframe. boom. done. but too much for me today I think.

#yeah no nevermind this is easy, check it out we just modify the titles code:
