...
subtitle: "or: Pat and the Great Suffering"
...
--
Pat!
- Myself and prior experience at Lilly
...
Limitations and hypothetical future work
...
<img src="interviewPresentation_files/patcollage.png" alt="" width="2000"/>
...
**... The issue is efficiency and stability!**
]
...
.pull-right[
- Macrocycle (large ring)!
- Highly variable!  
- Pyrrole subunits (4C-1N) x4
- methine bridges connecting
- resonant/aromatic/heterocyclic ring
- very good for e- transfer
]

Image from: https://commons.wikimedia.org/wiki/File:Porphin-18e.png
---

# Heme-b (focus of this condensed presentation)
...
- <input type="checkbox" checked> Bonding to Residues in Binding Pocket (Heme-c) </input>
--
...
Focuses on conformational differences induced by *binding to the environment*
...
--
at least, to design better hemoproteins to implement new or better chemical/industrial processes, and as potential therapeutics*.
...
Variation of environment by type of heme:
- Heme-b
- Heme-c
- Verdoheme
- Siroheme
...
*(Immenschuh, Stephan, et al. "Heme as a target for therapeutic interventions." Frontiers in pharmacology 8 (2017): 146.)
...
- Table to illustrate diverse protein set for HEM... (the rest in the back)

---

# Protein/Source Organism Data for HEM
...
UCSF-Chimera was used to extract or generate all data from PDBs in this study:
- AA Freq
- V
- SA, heme and pocket
- Distances
...
**Limitation:** only grab largest volume, no attempt to resolve "bubbles"
...
Solvent excluded (VDW) and solvent accessible surface areas of both the ligand and the binding pocket were calculated using Chimera's "surf" algorithm, which itself is an implementation of a program called MSMS 
...
1. Select ALL atoms **within 7A of heme** (see #5)
...
1. Introduces bias for residues on the fringes, but still orders residues by their respective distances and reduces noise from distant AA. 

--

1. Alternative Method: **za->zr**, select all atoms of residues within 7A (**zr**), v. all atoms within 7A (**za**)
...
Output from Python script, UCSF Chimera:
```
...
This framework can be used for other proteins and - ideally planar- ligands!
...
# (Focusing on heme-b, siroheme highlights)
...
.left-column[
- LOTS of nonpolar!
- therefore polar insufficient to explain binding
- at least nonpolar important, if not more!
]
...
.left-column[
- LEU, PHE, ILE large nonpolar
- ALA and VAL small nonpolar
- pyrole, methyl, vinyl interactions 
]
...
.left-column[
- GLY
- structural/no binding, just here to achieve shape
- **skip ahead**, return to remaining hypotheses if desired
]
...
.left-column[
- ASN and GLN (asparagine and glutamine)
- medium-size polar
- but not charged
- bang for buck
]
...
.left-column[
- CYS
- may coordinate Fe too!
- Sample used? But only for coordinating Res
- likely poor for interaction
]
...
.left-column[
- **Limitation:** did not perform "relative frequency" calculation
- unclear if this is useful comparison
- LEU and ALA populous here too
- after those, different
- GLY in high freq
]
...
- CYS, HIS, TYR
- CYS and HIS coord
- HIS interactions w. porphyrin
- TYR: propionates, redox/e- transfer
]
...
- LOTS of polar
- ARG (e+)
- salt bridges and interactions w carboxyl and propionates
]
...
.left-column[
- GLY
- maybe structural role; extend backbone and fit in more polars?
- **skip ahead**, come back if question on remaining hypotheses
]
...
- ALA, PHE, VAL
- nonpolar
- still have porphyrin ring and methyls
]
...
- polar, e-
- Repulsion?
- heads up: after this no clear trends
]
...
- LEU, MET, ILE, PRO
- nonpolars
- disfavored, but unclear why; other nonpolars preferred
]
...
.left-column[
- HIS
- other strongly polar, less bulky preferred?
]
...
.left-column[
- TYR, GLU
- GLU a little too big? Why ASP preferred?
- bang for buck
- TYR not needed for chemistry, no redox here
]
???
...
.left-column[
- very different from binding pocket
- ARG in pocket v. LEU in monomer
]
...
.left-column[
- Cys bridging/covalently bound
- n = 9, poor sample size/graph
]
...
.left-column[
- For all hemes, ~1200 A³ in binding pocket
- Others in appendix
]
...
.left-column[
- Similar to HEM, 10,000-11,000 A²
- useful to compare next slides
]
...
.left-column[
- ~7500 A²
- covalent bonds/thioether bonds exclude water?
- othewise similar to HEM
]
...
.left-column[
- ~21000 A²
- ligand SA is similar to HEM/HEC/VER etc.
- everything polar = no need to bury anything
- favorable interactions with solvent
]
...
1. Framework developed here is highly adaptable and may be useful in other investigations!
...
# Summing up Limitations 

- Small sample sizes
 - Limited availability of PDBs for some hemes
 - Only selected chain A of ea. molecule
 
--

- Inaccuracy/bias of predictive algorithms (V, SA, angles) e.g. Surfnet

--

- Use of Fe as the "center" of the heme molecule to calculate angles/distances

--

- Limited analysis
 - Proposed future analyses next slide

---

# Future Work/New Directions

- More thorough analyses
  - Get relative frequencies
  - Drill down on angles/distances
  - More thorough statistical analyses with greater sample size

--

- Integrate this structural data with chemical, e.g. predicted or measured binding affinity

- in silico or in the lab, see how well modified hemoproteins function or remain stable

- With enough datapoints, build a model to determine candidate molecules, integrating all the above data 
...
- heme-b interactive aa

- Heme C and Verdoheme analyses; full siroheme analyis

- remaining tables/graphs

---

# HEM AA Freq Interactive

```{r HEM-AAfreqINT,fig.cap="HEM2: AA Frequency within 7A",echo=FALSE, message= FALSE, warning=FALSE, results=FALSE}
load("mainData.RData")
activeLigand = "HEM"
library(dplyr) 
library(data.table)
...
g <- ggplot(eval(parse(text=paste(activeLigand,"_aaFreqDf",sep=""))),aes(x= reorder(Residue,-Freq),y=Freq))  +
      ...
      theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
p <- ggplotly(g)
htmltools::save_html(p, file = "HEMAA2.html")

```

<iframe src="HEMAA2.html" width="800" height="500"></iframe>

---

```{r HEM-AAdistINT,fig.cap="HEM2: Residue Distribution by Distance",echo=FALSE, message= FALSE, warning=FALSE, results=FALSE}
load("mainData.RData")
activeLigand = "HEM"
library(dplyr) 
library(data.table)
...
eval(parse(text=paste(activeLigand,"_distList$mean_distances",sep=""))) %>%
  ...
  # print(distanceDist)
   ppp <- ggplotly(distanceDist)
htmltools::save_html(ppp, file = "HEMAADist2.html")
   
```

<iframe src="HEMAADist2.html" width="800" height="500"></iframe>
...
.left-column[
- LEU and ALA high freq
- similar to HEM
- GLY and CYS higher freq
- CYS covalent bonds
- GLY ???
- LYS overlooked in thesis
- weakly polar, e+
- OK in HEC w/ covalent
- or special!
]
...
.left-column[
- Dissimilar to HEM/HEC
- LEU and ALA
- TYR and ARG; need more polar to convert?
- TYR for redox, oxygenase
- GLY, seems a role?
- After this ??
]

???
The frequency of amino acids in the binding pocket for verdoheme is dissimilar from heme-b and heme-c above. This is fairly surprising, given that verdoheme is an intermediate in the binding pocket for heme-b within heme oxygenases. 
