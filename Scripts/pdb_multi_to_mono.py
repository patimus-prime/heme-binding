...
# that may compose one subunit, if a subunit is made up of multiple chains. Let me check the big bois
# YES, AND THE SAME COMMMAND SEEMS TO WORK FOR MONOMERS OR MULTIMERS. SO WE CAN AUTOMATE THIS PROCESS
# well, seems the A thing holds true for everything I've seen so far. Here we go:
...
# specifying path:
path = "~/0_Pat_Project/test_folder/pdb_folder"
full_path = os.path.expanduser(path)

# specifying path to the results folder!
monomers_path = "~/0_Pat_Project/test_folder/monomers"
...
for fn in file_names:
    ...
    ################################################################################################
    # XXXXXXXXXXXXXXXXXX HERE'S WHERE THE CODE DIFFERS FROM OTHER XXXXXXXX ############3####33333
    ############################################################################################
    ...
    # this is in fact the case, go to reply log and remove that shit from the pdbs. Relaunch script
    rc("pause")

    # now save the monomer:
    # this looks funky but it' just within results_path, with processed_file.txt being saved
    rc("write format pdb 0 ~/0_Pat_Project/test_folder/monomers/%s" %(fn + ".mono.pdb"))
